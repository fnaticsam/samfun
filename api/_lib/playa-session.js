// Independent Playa gate. Never share token labels, cookies or secrets with /dev.
const crypto = require("crypto");

const SESSION_TTL_MS = 90 * 24 * 60 * 60 * 1000; // 90 days: a device logs in once, enforced server-side
const CLOCK_SKEW_MS = 60 * 1000;
const COOKIE = "playa_session";
const COOKIE_ATTRS = "Path=/playa; HttpOnly; Secure; SameSite=Lax";
...
function sign(secret, issuedAt) {
  return crypto.createHmac("sha256", secret).update(`playa-session-v1|${issuedAt}`).digest("hex");
}
...
// Requests from an allow-listed public address (PLAYA_TRUSTED_IPS, comma- or
...
function trustedIps() {
  return (process.env.PLAYA_TRUSTED_IPS || "").split(/[\s,]+/).map((s) => s.trim()).filter(Boolean);
}
...
function wantsLogout(url) {
  try {
    return new URL(url || "/", "http://localhost").searchParams.has("logout");
  } catch {
    return false;
  }
}

// The single admission rule for every /playa surface: a valid session cookie, or
